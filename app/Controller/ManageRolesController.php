...
public function add_superadmin()
{
	if ($this->request->is('post') && $this->request->data['ManageRole']['staff_id'] != 0) {
		...
		if ($this->ManageRole->save($this->request->data)) {
			...
			$this->loadModel('User');
			$staffid = $this->request->data['ManageRole']['staff_id']; 
			...
			if ($this->UserRole->save($this->request->data)) { 
				$this->Session->setFlash(__('The Super Admin has been saved.') , 'alert', array('class' => 'alert-success'
				));
			}
		}
		else {
...
public function add_developer_admin()
{
	if ($this->request->is('post') && $this->request->data['ManageRole']['staff_id'] != 0) {
		...
		if ($this->ManageRole->save($this->request->data)) {
			...
			$data = $this->User->find('first', ['conditions' => ['User.staff_id' => $staffid]]); // finding that id in USER table
			...
			if ($this->UserRole->save($this->request->data)) { // save data in USERROLE table
				$this->Session->setFlash(__('The  Admin has been saved.') , 'alert', array('class' => 'alert-success'
				));
			}
		}
		else {
	...
	$departments = [];
	$staffs = [];
	$roles = $this->ManageRole->Role->find('list', array('conditions' => array('Role.id' => array(Configure::read('stadmin'),Configure::read('tpadmin'),Configure::read('fbadmin')))));
...
public function add_admin()
{
	if ($this->request->is('post') && $this->request->data['ManageRole']['staff_id'] != 0) {
		...
		if ($this->ManageRole->save($this->request->data)) {
			...
			$this->Session->setFlash(__('The  Admin could not be saved. Please, try again.') , 'alert', array('class' => 'alert-success'
			));
		}
	}
	...
	$instid = $this->ManageRole->Staff->find('first', array(
		'fields' => array('Staff.institution_id') ,
		'conditions' => array('Staff.id' => $userid)));

	$departments = $this->ManageRole->Department->find('list', array(
...
public function add_developer_deptcoord(){
		if($this->request->is('post') && $this->request->data['ManageRole']['staff_id'] != 0){
			...
			if ($this->ManageRole->save($this->request->data)){	
			    		 ...
			    		 $staffid = $this->request->data['ManageRole']['staff_id']; //staff which we have selected itz id is stored in variable staffid
			   			 $data1 = $this->ManageRole->Role->UserRole->User->find('first',['conditions'=>['User.staff_id'=>$staffid]]); // finding that id in USER table
			    		 $this->request->data['UserRole']['user_id'] = $data1['User']['id']; // appending the user id after findind itz User
			   			 $this->request->data['UserRole']['role_id'] = $this->request->data['ManageRole']['role_id'];//copy role_id value from managerole to userrole table
				...
				unset($this->request->data);
                    ...
                    $departments = [];
           			$staffs = [];
           			$roles = $this->ManageRole->Role->find('list',array( 'conditions'=>array('Role.id'=>array(Configure::read('deptcoordinator')))));
         			$this->set(compact('institutions', 'departments', 'staffs','roles'));
    }
